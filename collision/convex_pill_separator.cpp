...
//
// C++ Implementation: convex_pill_separator
//
// Description: 
//
//
...
//
//
#include "convex_pill_separator.h"
#include "convex_polygon_volume.h"
#include "pill_volume.h"
#include "contact_solver.h"

bool PillConvexSeparator::handles( const Volume& p_A, const Volume& p_B ) const {

	return (p_A.get_volume_type()==Volume::CONVEX_POLYGON && p_B.get_volume_type()==Volume::PILL) || (p_A.get_volume_type()==Volume::PILL && p_B.get_volume_type()==Volume::CONVEX_POLYGON);
	
}

void PillConvexSeparator::separate( const Volume& p_A,const Matrix4& p_A_transform, const Volume& p_B, const Matrix4 &p_B_transform, VolumeContactHandler *p_contact_handler) const {
	...
	if (p_A.get_volume_type()==Volume::CONVEX_POLYGON && p_B.get_volume_type()==Volume::PILL) {
	
		A=SAFE_CAST<const ConvexPolygonVolume*>(&p_A);
		B=SAFE_CAST<const PillVolume*>(&p_B);
		...
		B=SAFE_CAST<const PillVolume*>(&p_A);
...
//		float bdold=best_depth;
	...
	Vector3 pill_up=Vector3(0,B->get_height()/2.0 ,0);
	B_transform->transform_no_translation( pill_up );
	...
	for (int s=0;s<2;s++) {
		...
		Vector3 center=B_transform->get_translation()+((s==0)?pill_up:-pill_up);
